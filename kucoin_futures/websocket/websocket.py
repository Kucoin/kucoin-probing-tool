...
class ConnectWebsocket:
    ...
    async def _run(self, event: asyncio.Event):
        ...
        async with websockets.connect(self.get_ws_endpoint(), ssl=self.get_ws_encryption(),sock=self._sock) as socket:
            ...
            while keep_alive:
                # Now we ping every 3s to get the pong so we can test the latency of
                # websocket
                if time.time() - self._last_ping > 3:
                    await self.send_ping()
                try:
                    ...
                    try:
                        ...
                        # is pong?
                        if 'type' in msg:
                            if msg['type'] == 'pong' and 'timestamp' in msg:
                                # Local RTT
                                local = time.time() * 1000000
                                self._last_ping_pong_rtt = local - int(msg['id'])
                                # pong has a timestamp 
                                
                                print("ping rtt", self._last_ping_pong_rtt / 2, local - msg['timestamp'])
                        msg['ws_rtt_us'] = self._last_ping_pong_rtt
    ...
    def get_ws_endpoint(self):
        ...
        ws_endpoint = f"{endpoint}?token={token}&connectId={ws_connect_id}&acceptExtras=True"
        return ws_endpoint
